/* Blossom & Blade — chat.js (bb4)
   - Waits for DOMContentLoaded (fixes early-load wiring)
   - Robust selectors so Send always works
   - Persona greets first
   - Trial/paywall completely disabled (flag)
   - Captions nuked globally
   - Background pop and mapping
   - Phrase updates (surrender / daring / worked up; Dylan backpack; Silas rhyme)
*/

document.addEventListener('DOMContentLoaded', () => {
  const TRIAL_ENABLED = false;      // OFF now. Flip true later.
  const TRIAL_MINUTES = 10;         // used only if enabled
  ...
  const chat = qs('.chat') || document.body;
  const messagesEl = qs('.messages') || (() => { const d=document.createElement('div'); d.className='messages'; chat.prepend(d); return d; })();
  const composer = qs('#composer') || qs('.composer') || qs('form');
  const inputEl = qs('#composer-input') || qs('textarea', composer) || qs('input[type="text"]', composer) || qs('input', composer);
  const sendBtn = qs('#send-btn') || qs('.send', composer) || qs('button[type="submit"]', composer) || qs('button', composer);
  ...
  // --- Caption/label killer (global, not just inside .card) ---
  function nukePortraitText(){
    document.querySelectorAll('*').forEach(el=>{
      const t = (el.textContent || '').trim().toLowerCase();
      if (el.tagName === 'FIGCAPTION' || /—\s*portrait/.test(t)) {
  ...
  function ensureTrialStart(){
    if (!TRIAL_ENABLED){ try { localStorage.removeItem(trialKey); } catch{} return; }
    if (!localStorage.getItem(trialKey)) localStorage.setItem(trialKey, String(Date.now()));
  }
  function msLeftInTrial(){
    if (!TRIAL_ENABLED) return Infinity;
    const s = localStorage.getItem(trialKey); if (!s) return TRIAL_MINUTES*60*1000;
    const left = TRIAL_MINUTES*60*1000 - (Date.now() - Number(s));
    return Math.max(0, left);
  }
  function blockIfTrialExpired(){
    if (!TRIAL_ENABLED) return false;
    if (msLeftInTrial() <= 0){
      if (sendBtn) sendBtn.disabled=true; if (inputEl) inputEl.disabled=true;
  ...
  function addMessage(role, content){
    ...
    const stick = atBottom(messagesEl); messagesEl.appendChild(row);
    if (stick) row.scrollIntoView({behavior:'smooth', block:'end'});
    history.push({role, content}); log(role+':', content);
  }
  ...
  function avoidRepeat(text){
    for (const prev of lastAssistant()) if (semSame(prev, text)) return text + " (mm—come closer.)";
    return text;
  }
...
One assent ("yes", "please", "yes sir/ma’am", "make me", "good girl", "I want that") → escalate confidently.
...
Keep responses varied; avoid repeating recent wording.`;
  ...
  async function callModel(userText){
    ...
    }catch(e){ log('api err', e?.message||e); return fallbackLine(true); }
  }
  function fallbackLine(err){ if (err) return "Connection snag—come closer and say that again, pretty thing.";
  ...
  // Greet on load
  function greet(){
    const hasAssistant = !!qs('.messages .assistant');
    if (!hasAssistant){
      const ops = phraseBank.openers[man]; if (ops && ops.length) addMessage('assistant', ops[Math.floor(Math.random()*ops.length)]);
    }
  }

  // Send
  async function onSend(){
    if (TRIAL_ENABLED && msLeftInTrial() <= 0){ if (blockIfTrialExpired()) return; }
  ...
  // Wire events (works regardless of exact markup)
